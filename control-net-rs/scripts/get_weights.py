...
def ensure_data_dir(safetensors):
    ...
    if os.path.exists(data_path):
        ...
        newfiles = [
            x for name in safetensors for x in (f"{name}.bin", f"{name}.safetensors")
        ]
...
def get_safetensors(safetensors, weight_bits):
    for name, url in safetensors.items():
        ...
        model = torch.load(
            os.path.join(data_path, f"{name}.bin"), map_location=torch.device("cpu")
        )
        tensors = (
            {k: v.clone().detach() for k, v in model.items() if "text_model" in k}
            if name in ["clip_v2.1", "clip_v1.5"]
            else dict(model)
        )
...
def get_controlnet_safetensors(url, output_filename):
    print(f"Getting control net tensors...")
    urllib.request.urlretrieve(url, os.path.join(data_path, output_filename))


def get_vocab(vocab_url):
    print("Getting vocab...")
    urllib.request.urlretrieve(
        vocab_url, os.path.join(data_path, f"{vocab_filename}.gz")
    )
    import gzip
...
def get_urls(sd_version, weight_bits):
    ...
    safetensors_v1_5 = {
        "vae_v1.5": f"https://huggingface.co/runwayml/stable-diffusion-v1-5/resolve/{branch}/vae/diffusion_pytorch_model.bin",
        "unet_v1.5": f"https://huggingface.co/runwayml/stable-diffusion-v1-5/resolve/{branch}/unet/diffusion_pytorch_model.bin",
        "clip_v1.5": f"https://huggingface.co/openai/clip-vit-large-patch14/resolve/main/pytorch_model.bin",
        "controlnet": f"https://huggingface.co/lllyasviel/sd-controlnet-canny/resolve/main/diffusion_pytorch_model.bin",
    }
    safetensors_v2_1 = {
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sd_version", "-v", choices=["2.1", "1.5"], default="1.5")
    parser.add_argument("--weight_bits", "-w", choices=["16", "32"], default="16")
    args = parser.parse_args()

    if args.sd_version == "2.1":
        raise NotImplementedError("ControlNet currently only supports 1.5")

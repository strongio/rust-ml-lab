...
struct Args {
    ...
    #[arg(
        long,
        value_name = "FILE",
        default_value = "data/unet_v1.5.safetensors"
    )]
    unet_weights: String,
    ...
    #[arg(
        long,
        value_name = "FILE",
        default_value = "data/controlnet.safetensors"
    )]
    ...
    #[arg(
        long,
        value_name = "FILE",
        default_value = "data/clip_v1.5.safetensors"
    )]
    clip_weights: String,

    /// The VAE weight file, in .ot or .safetensors format.
    #[arg(long, value_name = "FILE", default_value = "data/vae_v1.5.safetensors")]
    vae_weights: String,
    ...
    fn image_preprocess<T: AsRef<std::path::Path>>(&self, path: T) -> anyhow::Result<Tensor> {
        match self {
            Self::Canny => {
                ...
                tch::vision::image::save(&tensor.squeeze(), "media/edges.png").unwrap();
...
fn run(args: Args) -> anyhow::Result<()> {
    ...
    println!("Building VAE autoencoder");
